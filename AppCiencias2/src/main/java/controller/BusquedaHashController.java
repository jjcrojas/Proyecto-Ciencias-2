...
public class BusquedaHashController {
    ...
    private void insertarClave() {
        ...
        // No repetición (revisar en toda la estructura: slots + colisiones)
        if (existeClaveEnTabla(claveTxt) || existeClaveEnColisiones(claveTxt)) {
            resultadoLabel.setText("Esa clave ya existe (insertada o pendiente por resolver).");
            limpiarInsercion();
            return;
        }
...
private void buscarClave() {
    ...
    // 0) Búsqueda global defensiva: evita falsos "No encontrada" si cambió MOD/hash/estrategia
    SlotHash slotGlobal = encontrarSlotConClave(claveTxt);
    if (slotGlobal != null) {
        long fin = System.nanoTime();
        tabla.getSelectionModel().select(slotGlobal);
        tabla.scrollTo(slotGlobal);
        resultadoLabel.setText("Encontrada en posición " + slotGlobal.getPosicion()
                + " | (búsqueda global)"
                + " | Comparaciones: " + comparaciones
                + " | Tiempo: " + (fin - inicio) + " ns");
        limpiarBusqueda();
    ...
    SlotHash bucketGlobal = encontrarSlotConColision(claveTxt);
    if (bucketGlobal != null) {
        long fin = System.nanoTime();
        tabla.getSelectionModel().select(bucketGlobal);
        tabla.scrollTo(bucketGlobal);
        resultadoLabel.setText("Encontrada en colisiones de la posición " + bucketGlobal.getPosicion()
                + " | (búsqueda global)"
                + " | Comparaciones: " + comparaciones
                + " | Tiempo: " + (fin - inicio) + " ns");
        limpiarBusqueda();
    ...
    // 1) Revisar el slot base según hash actual
    SlotHash base = data.get(start);
    comparaciones++;
...
if (base.getColisiones() != null && !base.getColisiones().isEmpty()) {
    ...
    @FXML
        ...
        private boolean existeClaveEnColisiones(String claveTxt) {
            return encontrarSlotConColision(claveTxt) != null;
        }

        /** Devuelve el slot principal que contiene la clave, o null si no existe. */
        private SlotHash encontrarSlotConClave(String claveTxt) {
            return data.stream()
                    .filter(slot -> claveTxt.equals(slot.getClave()))
                    .findFirst()
                    .orElse(null);
        }

        /** Devuelve el bucket cuya lista de colisiones contiene la clave real, o null si no existe. */
        private SlotHash encontrarSlotConColision(String claveTxt) {
            return data.stream()
                    .filter(slot -> slot.getColisiones() != null
                            && slot.getColisiones().stream()
                            .map(this::extraerClaveReal)
                            .anyMatch(claveTxt::equals))
                    .findFirst()
                    .orElse(null);
